...
function Formmeat(){
    ...
    useEffect( () => {
        axios.get('http://localhost:4000/items-category',{
          ...
          useEffect(() => {
            ...
            navigate('/formothers', {state: data})
            }
          }, [isFormIncomplete]);
    ...
    return(
        <section>
            ...
            <h1 class="text-warning">List of availbale Meat</h1>
            <section><h2 class="text-danger">{empty? 'We are out of Meat' : <>Max {maximum} Meat ({maximumf} for families)</>}</h2></section>
